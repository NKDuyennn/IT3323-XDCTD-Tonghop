 ...
 */
/*
 * Ham compileBlock() - BIEN DICH MOT KHOI CODE (CU PHAP MOI)
 * Cu phap moi: <Block> ::= <Declarations> <Block2>
 * 
 * Khong con phan tang cung nhu cu nua, thay vao do:
 *   - compileDeclarations(): xu ly tat ca khai bao (CONST, TYPE, VAR) co the xen ke
 *   - compileBlock2(): xu ly SubDecls va BEGIN...END
 */
void compileBlock(void) {
  assert("Parsing a Block ....");
  
  compileDeclarations();             // Xu ly tat ca cac khai bao
  compileBlock2();                   // Xu ly Function/Procedure va BEGIN...END
  
  assert("Block parsed!");
}

/*
 * Ham compileDeclarations() - XU LY TAT CA KHAI BAO (CU PHAP MOI)
 * Cu phap: <Declarations> ::= <ConstDecls> <Declarations>
 *                            | <TypeDecls> <Declarations>
 *                            | <VarDecls> <Declarations>
 *                            | ε (rong)
 * 
 * CHO PHEP XEN KE: co the co CONST, roi VAR, roi lai CONST,...
 * Vi du hop le:
 *   CONST x = 5;
 *   VAR a : INTEGER;
 *   CONST y = 10;    <- Quay lai CONST duoc!
 *   TYPE T = INTEGER;
 */
void compileDeclarations(void) {
  // De quy: lap lai trong khi gap CONST, TYPE, hoac VAR
  switch (lookAhead->tokenType) {
  case KW_CONST:
    compileConstDecls();             // Xu ly CONST
    compileDeclarations();           // De quy: tiep tuc xu ly khai bao khac
    break;
    
  case KW_TYPE:
    compileTypeDecls();              // Xu ly TYPE
    compileDeclarations();           // De quy
    break;
    
  case KW_VAR:
    compileVarDecls();               // Xu ly VAR
    compileDeclarations();           // De quy
    break;
    
  default:
    // Khong gap CONST, TYPE, VAR -> ket thuc phan Declarations (epsilon)
    break;
  }
}
/*
 * Ham compileBlock2() - XU LY SUBDECLS VA BLOCK3 (CU PHAP MOI)
 * Cu phap: <Block2> ::= <SubDecls> <Block3>
 *                     | <Block3>
 */
void compileBlock2(void) {
  assert("Parsing Block 2 ....");
  
  // Kiem tra xem co SubDecls (Function/Procedure) khong
  if (lookAhead->tokenType == KW_FUNCTION || lookAhead->tokenType == KW_PROCEDURE) {
    compileSubDecls();               // Xu ly Function/Procedure
  }
  
  compileBlock3();                   // Luon chuyen sang BEGIN...END
  
  assert("Block 2 parsed");
}

/*
 * Ham compileBlock3() - XU LY BEGIN...END (CU PHAP MOI)
 * Cu phap: <Block3> ::= KW_BEGIN <Statements> KW_END
 * 
 * Day la noi chua than chuong trinh (cac cau lenh thuc thi)
 */
void compileBlock3(void) {
  assert("Parsing Block 3 ....");
  
  eat(KW_BEGIN);                     // Bat buoc bat dau bang BEGIN
  compileStatements();               // Bien dich cac cau lenh
  eat(KW_END);                       // Bat buoc ket thuc bang END
  
  assert("Block 3 parsed");
}

// ============= LOAI BO BLOCK4 VA BLOCK5 - KHONG CON CAN THIET =============
// Chuc nang da duoc tich hop vao compileBlock2() va compileBlock3()

// ============= HAM XU LY KHAI BAO HANG SO (CONST) =============

/*
 * Ham compileConstDecls() - BIEN DICH KHAI BAO CONST (CU PHAP MOI)
 * Cu phap: <ConstDecls> ::= KW_CONST <ConstDecl> <ConstDecls2>
 * 
 * An KW_CONST, sau do PHAI CO IT NHAT 1 khai bao,
 * roi xu ly cac khai bao con lai qua compileConstDecls2()
 */
void compileConstDecls(void) {
  eat(KW_CONST);                     // An tu khoa CONST
  compileConstDecl();                // PHAI CO it nhat 1 khai bao
  compileConstDecls2();              // Cac khai bao con lai (co the rong)
}

/*
 * Ham compileConstDecls2() - XU LY CAC KHAI BAO CONST TIEP THEO (CU PHAP MOI)
 * Cu phap: <ConstDecls2> ::= <ConstDecl> <ConstDecls2>
 *                           | ε (rong)
 * 
 * De quy: neu con TK_IDENT thi tiep tuc, khong thi dung (epsilon)
 */
void compileConstDecls2(void) {
  if (lookAhead->tokenType == TK_IDENT) {
    compileConstDecl();              // Bien dich 1 khai bao
    compileConstDecls2();            // De quy: tiep tuc cac khai bao con lai
  }
  // Neu khong phai TK_IDENT -> ket thuc (epsilon)
}
...
/*
 * Ham compileTypeDecls() - BIEN DICH KHAI BAO TYPE (CU PHAP MOI)
 * Cu phap: <TypeDecls> ::= KW_TYPE <TypeDecl> <TypeDecls2>
 */
void compileTypeDecls(void) {
  eat(KW_TYPE);                      // An tu khoa TYPE
  compileTypeDecl();                 // PHAI CO it nhat 1 khai bao
  compileTypeDecls2();               // Cac khai bao con lai
}

void compileTypeDecls2(void) {
  if (lookAhead->tokenType == TK_IDENT) {
    compileTypeDecl();               // Bien dich 1 khai bao
    compileTypeDecls2();             // De quy
  }
  // Epsilon
}
...
/*
 * Ham compileVarDecls() - BIEN DICH KHAI BAO VAR (CU PHAP MOI)
 * Cu phap: <VarDecls> ::= KW_VAR <VarDecl> <VarDecls2>
 */
void compileVarDecls(void) {
  eat(KW_VAR);                       // An tu khoa VAR
  compileVarDecl();                  // PHAI CO it nhat 1 khai bao
  compileVarDecls2();                // Cac khai bao con lai
}

void compileVarDecls2(void) {
  if (lookAhead->tokenType == TK_IDENT) {
    compileVarDecl();                // Bien dich 1 khai bao
    compileVarDecls2();              // De quy
  }
  // Epsilon
}
